...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    temp_x = torch.nn.functional.pad(x, (conv_param['pad'], conv_param['pad'], conv_param['pad'], conv_param['pad']))
    ...
    for n in range(N):
      for f in range(F):
        for i in range(Hout):
          for j in range(Wout):
            temp1 = (temp_x[n, :, i*conv_param['stride']: i*conv_param['stride']+HH, j*conv_param['stride']:j*conv_param['stride']+WW]*w[f]).sum()
            temp2 = b[f]
            out[n, f, i, j] = temp1+temp2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
  ...
  def forward(x, pool_param):
    ...
    for  n in range(N):
      for c in range(C):
        for i in range(Hout):
          for j in range(Wout):
            out[n, c, i, j] = torch.max(x[n, c, i*pool_param['stride']:i*pool_param['stride']+pool_param['pool_height'], \
...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    c_out, c_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    h_out, h_cache = Linear_ReLU.forward(c_out, W2, b2)
    scores, s_cache = Linear.forward(h_out, W3, b3)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss += self.reg* (torch.sum(torch.square(W1)) + torch.sum(torch.square(W2))+ torch.sum(torch.square(W3)))
    
    h3_back, grads['W3'], grads['b3'] = Linear.backward(loss_grad, s_cache)
    grads['W3']+=self.reg*2*W3
    h2_back, grads['W2'], grads['b2'] = Linear_ReLU.backward(h3_back, h_cache)
    grads['W2']+=self.reg*2*W2
    h1_back, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(h2_back, c_cache)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    if weight_scale == 'kaiming':
      self.params['W1'] = kaiming_initializer(num_filters[0], C, K=3, dtype=dtype, device=device)
    else:
    ...
    for i in range(2, self.num_layers):
      if weight_scale == 'kaiming':
        self.params['W'+repr(i)] = kaiming_initializer(num_filters[i-1], num_filters[i-2], K=3, dtype=dtype, device=device)
      else:
        self.params['W'+repr(i)] = weight_scale*torch.randn(num_filters[i-1], num_filters[i-2], 3, 3, dtype=dtype, device=device)
      self.params['b'+repr(i)] = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
    
    if weight_scale == 'kaiming':
      self.params['W'+repr(self.num_layers)] = kaiming_initializer(num_filters[-1]*H*W//(4**len(self.max_pools)), num_classes, dtype=dtype, device=device)
    else:
      self.params['W'+repr(self.num_layers)] = weight_scale*torch.randn(num_filters[-1]*H*W//(4**len(self.max_pools)), num_classes, dtype=dtype, device=device)
