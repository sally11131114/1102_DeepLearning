...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    C=1+max(x)
    L=len(x)
    y = torch.zeros(L, C, dtype=torch.float32)
    idx = torch.arange(L)
    y[idx, x]=1
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    temp=x.view(2, 3, 4).permute(1, 0, 2).reshape(3, -1)
    y=temp
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    temp=x.clone()
    row_min_vals, row_min_idxs = temp.min(dim=1)
    idx1=torch.arange(temp.shape[0])
    temp[idx1, row_min_idxs]=0
    y=temp
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    temp=x.clone()
    mean=temp.sum(axis=0)/temp.shape[0]
    nor=(x-mean)**2
    nor=torch.sqrt(nor.sum(axis=0)/(temp.shape[0]-1))
    y=(x-mean)/nor
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
