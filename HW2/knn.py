...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(x_train_flat.shape[0]):
      for k in range(x_test_flat.shape[0]):
          dists[i][k]=torch.sqrt(torch.sum((x_train_flat[i]-x_test_flat[k])**2))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # same datatype and device as x_train
  num_train = x_train.s
  dists = x_train.new_zeros(num_train, num_test)norm (or its instance method variant), nor any       #
  # functions from torch.nn or torch.nn.functional.                            #
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_flat = x_train.clone().view(x_train.shape[0], -1)
  x_test_flat = x_test.clone().view(x_test.shape[0], -1)

  for i in range(x_train.shape[0]):
      temp = torch.sqrt(torch.sum((x_test_flat-x_train_flat[i])**2, axis=1)) 
      #sum預設column相加
      dists[i]=temp
  ##############################################################################
  #                             END OF YOUR CODE                               #
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_flat = x_train.clone().view(num_train, -1)
  x_test_flat = x_test.clone().view(num_test, -1)
  x_test_flat_t = x_test_flat.clone().t()

  sum1=torch.sum(torch.square(x_train_flat), axis=1)
  #print("sum1:", sum1.view(-1, 1).shape)
  prod=2*torch.mm(x_train_flat, x_test_flat_t)
  sum2=torch.sum(torch.square(x_test_flat), axis=1)
  #print("sum2:", sum2.shape)
  #print("sum+:", (sum1.view(-1, 1)+sum2.view(1, -1)).shape)
  #這裡的sum1和sum2必須用view強制reshape成size([100, 1]), size([1, 100])
  #不然得出的tensor一樣會是size([100])因而造成difference過大
  dists=torch.sqrt(sum1.view(-1, 1)+sum2.view(1, -1)-prod)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  dists_topk_values, dists_topk_idxs=torch.topk(dists_topk, k, largest=False, dim=0)
  
  for i in range(dists_topk_idxs.shape[1]):
    result=[]
    for k in range(dists_topk_idxs.shape[0]):
      result.append(y_train[dists_topk_idxs[k][i]])
      # dists_topk_idxs為利用torch.topk取出的前k個最小值之index
      # 接著利用這些index找出所對應的label(y_train的值即為label),
      # 並依照column加入list, 用max()找出符合規定的多數者
