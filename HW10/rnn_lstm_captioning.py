...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    print('x: ', x.shape)
    out = torch.mm(prev_h, Wh)+torch.mm(x, Wx)+b
...
def rnn_step_backward(dnext_h, cache):
    ...
    #N*H
    dprev_h = torch.mm(dnext_h, Wh.T) #Wh=(H, H) why Wh.T
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # Replace "pass" statement with your code
    N, T, D = x.shape
    h = torch.zeros(N, T, Wh.shape[0], dtype = torch.float64, device='cuda')
    cache = []
    next_h = h0
    for i in range(T):
        #next_h = (N, H). input x[:, i, :] = (N, D). h=(N, T, H)
        next_h, temp_cache = rnn_step_forward(x[:, i, :], next_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    dh_cur = torch.zeros(N, H , dtype = torch.float64, device='cuda')
    for i in range(T-1,-1,-1):
        dh_cur += dh[:, i, :]
        dx[:, i, :], dh_cur, dWx_cur, dWh_cur, db_cur = rnn_step_backward(dh_cur, cache[i])
        dWx += dWx_cur
        dWh += dWh_cur
        db += db_cur
    dh0 = dh_cur
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.feature = ImageEncoder(image_encoder_pretrained)
        self.featureProjector = nn.Linear(4, hidden_dim)
    ...
    def forward(self, images, captions):
        ...
        features = self.feature.forward(images)
