...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for next_channel in stem_channels:
            temp_conv = nn.Conv2d(cur_channel, next_channel, 3, 1, 1)
            temp_conv.bias.data.fill_(0.)
            nn.init.normal_(temp_conv.weight, 0, 0.01)
            stem_cls.append(temp_conv)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for p in feats_per_fpn_level.keys():
            ...
            centerness_logits[p] = centerness_logits[p].reshape(B, -1, H*W).transpose(1, 2)
...
def fcos_get_deltas_from_locations(
    ...
    deltas[:, 0] = locations[:, 0] - gt_boxes[:, 0] #l
    deltas[:, 1] = locations[:, 1] - gt_boxes[:, 1] #t
    deltas[:, 2] = gt_boxes[:, 2] - locations[:, 0] #r
    deltas[:, 3] = gt_boxes[:, 3] - locations[:, 1] #b
    deltas /= stride
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    up = torch.min(deltas[:, 0], deltas[:, 2]) * torch.min(deltas[:, 1], deltas[:, 3])
    down = torch.max(deltas[:, 0], deltas[:, 2]) * torch.max(deltas[:, 1], deltas[:, 3])
    centerness = torch.sqrt(up/down)
    mask = deltas[:, 0]==-1
    centerness[mask] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        temp_fpn = self.backbone(images)
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(temp_fpn)
        ######################################################################
        ...
        # Replace "pass" statement with your code
        s = dict()
        for p_name, feature in temp_fpn.items():
            s[p_name] = feature.shape
        locations_per_fpn_level = get_fpn_location_coords(s, self.backbone.fpn_strides, device = images.device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for temp in gt_boxes:
            matched_gt_boxes.append(fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, temp))
        ...
        for i in matched_gt_boxes:
            temp_p3 = fcos_get_deltas_from_locations(locations_per_fpn_level['p3'], i['p3'],  self.backbone.fpn_strides['p3'])
            temp_p4 = fcos_get_deltas_from_locations(locations_per_fpn_level['p4'], i['p4'],  self.backbone.fpn_strides['p4'])  
            temp_p5 = fcos_get_deltas_from_locations(locations_per_fpn_level['p5'], i['p5'],  self.backbone.fpn_strides['p5'])    
            matched_gt_deltas.append({'p3': temp_p3, 'p4': temp_p4, 'p5': temp_p5})  
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        N, H_W, C = pred_cls_logits.shape
        B, H_W, T = matched_gt_deltas.shape
