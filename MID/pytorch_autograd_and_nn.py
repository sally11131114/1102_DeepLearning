...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  H_out_1 = 1 + (H-kernel_size_1+2*2)/1
  ...
  H_out_2 = 1 + (H_out_1-kernel_size_2+2*1)/1

  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, channel_2*int(H_out_2)*int(H_out_2), dtype=dtype, device='cuda'))
